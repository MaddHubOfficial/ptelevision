		...
		while duiObj do
			if tvObj and sfHandle ~= nil and HasScaleformMovieLoaded(sfHandle) then
				...
				if screenModel.Target then
					...
					DrawScaleformMovie_3dSolid(sfHandle, offset, 0, 0, -hz, 2, 2, 2, scale * 1, scale * (9 / 16), 2);
				end;
			end;
			Citizen.Wait(0);
		end;
...
Citizen.CreateThread(function()
	while true do
		...
		for i = 1, #Locations do
			...
			if not Locations[i].obj and dist < 20 then
			...
			elseif Locations[i].obj and dist > 20 then
