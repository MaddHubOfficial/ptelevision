...
function BroadcastMenu()
	...
	local input = lib.inputDialog("Live Broadcast", {
		"Channel Name:",
		"Broadcast URL:"
	});
	if input[1] and input[2] then
		TriggerServerEvent("ptelevision:broadcast", {
			name = input[1],
			url = input[2]
		});
	end;
...
function WebBrowserMenu()
	lib.hideMenu();
	local input = lib.inputDialog("Web Browser", {
		"URL:"
	});
	if input then
		TriggerServerEvent("ptelevision:event", CURRENT_SCREEN, "ptv_status", {
...
function OpenTVMenu()
	...
	lib.registerMenu({
		...
		options = {
			...
			{
				label = "TV Channels",
				values = ChannelList.display,
				description = "Live TV Channels in San Andreas!",
				defaultIndex = ChannelList.current
			},
			{
